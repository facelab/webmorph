...
plot(temlist, pt.plot = TRUE, line.plot = TRUE)
```
...
```{r resize, fig.cap="Resized Images and Templates"}
temlist %>%
  resize(.1) %>%
  plot()
```
...
You can use `rep()` to repeat images in a temlist. Here, we repeat the faces 3 times each, apply 6 different rotations with different background colours, crop them to the same size, and plot them with 6 different template point colours.
